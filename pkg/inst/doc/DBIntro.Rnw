...
<<dbputraw>>=
db_put(dbh,key=charToRaw("Ross"),charToRaw("Ihaka"))
charToRaw("Ihaka")
@
...
Using cursors, or even explaining much beyond this is outside the scope
of this document, but the above description should be sufficient to understand
what is happening internally.

To add a record with a cursor, we first need a cursor handle to our
...
the creation call.  From here the cursor (mapping to the C API struct {\bf DBC})
is ready for use.

To put a record in the database, we simply use the appropriate {\emph cursor} method:

<<dbcursorput>>=
dbcursor_put(dbc, key=100L, value=5L, flags=mkFlags(DB_KEYLAST))
@
