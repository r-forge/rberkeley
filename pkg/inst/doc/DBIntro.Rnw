...
db_open(dbh, txnid=NULL, file="myDB.db", type="BTREE", flags=mkFlags(DB_CREATE, DB_EXCL))
@

The first line loads the RBerkeley package into R.  From there
we need to create a handle to open a database.  This can take some
additional parameters, but we use the defaults here.  The object returned
is of class {\tt DB}.

{\tt db\_open} actually opens the underlying database.  Once again,
different flags may be passed in, as well as other parameters that will
be covered in later documentation, but for now this will suffice for
the introductory example.

One interesting RBerkeley function is {\tt mkFlags}; this provides
access to the internal constants defined by DB for use as flag
parameters to be passed to many of the internal functions. The C
API allows for flags to be constructed via bitwise OR operations
...
into C code that performs this bitwise operation for us. mkFlags thus
...
mkFlags(DB_CREATE,DB_EXCL,DB_EXCL)
<<mkFlagsWithWarning>>=
mkFlags(DB_CREATE,DB_WRONG)
@
<<mkFlagsWarning, echo=FALSE>>=
withCallingHandlers( {
mkFlags(DB_CREATE,DB_WRONG)
}, warning=function(w){ assign("last.warning", w, env=baseenv())})
@
last.warning
@
