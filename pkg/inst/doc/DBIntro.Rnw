...
relational database is that DB have no notion of type.  Records, both
...
allows for a preprocessing model to match any arbitrary schema that is chosen.
...
sent to the database.  The only exception is for the object type {\tt raw}
which is passed as is.  This convention allows for maximal ease of use
from within R, but offers the benefit of providing a direct interface
to the underlying flexibility of DB.  All DB data must be

\subsubsection{DB->put aka db\_put}
